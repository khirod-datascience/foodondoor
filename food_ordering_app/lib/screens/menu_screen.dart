...
class _MenuScreenState extends State<MenuScreen> {
  ...
  Future<void> _fetchRestaurantDetails() async {
    ...
    try {
      ...
      final url = '${AppConfig.baseUrl}/restaurants/${widget.restaurantId}/'; // API endpoint for ONE restaurant
      debugPrint('Fetching restaurant details from: $url');
      final response = await dio.get(url);
      debugPrint('Restaurant Details Response (${response.statusCode}): ${response.data}');
      
      if (response.statusCode == 200) {
        if (response.data is Map) {
          // If API returns a Map directly (Correct way)
          _restaurantDetails = Map<String, dynamic>.from(response.data);
        } else if (response.data is List && (response.data as List).isNotEmpty) {
           // If API incorrectly returns a List, try to find the matching restaurant
           debugPrint('Warning: Restaurant details API returned a List instead of a Map. Attempting to find match.');
           final listData = response.data as List;
           // Find the first item in the list that matches the requested ID (vendorId)
           var foundRestaurant = listData.firstWhere(
              (item) => item is Map && item['vendor_id']?.toString() == widget.restaurantId,
              orElse: () => null, // Return null if no match found
           );
           if (foundRestaurant != null) {
              _restaurantDetails = Map<String, dynamic>.from(foundRestaurant);
              debugPrint('Found matching restaurant in the list.');
           } else {
             _error = 'Restaurant details not found in the returned list.';
             debugPrint('Could not find restaurant with vendorId ${widget.restaurantId} in the returned list.');
           }
        } else {
          // Handle other unexpected data types or empty list
          _error = 'Unexpected data format received for restaurant details.';
          debugPrint('Restaurant details API returned unexpected data type: ${response.data?.runtimeType}');
        }
      } else {
         _error = 'Failed to load restaurant details. Status: ${response.statusCode}';
         debugPrint('Restaurant details fetch failed with status: ${response.statusCode}');
      }
    } on DioException catch (e) {
        _error = 'Error loading details: ${e.response?.data?['detail'] ?? e.message}';
        debugPrint('DioError fetching restaurant details: $e');
    } catch (e) {
      _error = 'An unexpected error occurred loading details.';
  ...
  Future<void> _fetchFoodItems() async {
    ...
    try {
      ...
      final url = '${AppConfig.baseUrl}/food-listings/${widget.restaurantId}/'; // Endpoint for menu items
